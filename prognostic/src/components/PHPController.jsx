...
class PHPController {
  ...
  deletePerson(person) {
    for (var i = 0; i < this.personTable.length; i++) {
      if (
        this.personTable[i]["Id"] == person["Id"] &&
        person["Flag"] == "D" &&
        person["Id"].charAt(0) == "T"
      ) {
        //delete a person not exists in database
        ...
        //delete a person exists in the database
  ...
  updateProject(project) {
    for (var i = 0; i < this.projectTable.length; i++) {
      if (
        this.projectTable[i]["Id"] == project["Id"] &&
        project["Flag"] == "U"
      ) {
  ...
  updateAllocation(allocation) {
    for (var i = 0; i < this.allocationTable.length; i++) {
      if (
        this.allocationTable[i]["Id"] == allocation["Id"] &&
        allocation["Flag"] == "U"
      ) {
  ...
  saveAll() {
    ...
    for (var i = 0; i < this.personTable.length; i++) {
      if (this.personTable[i]["Flag"] == "I") {
        var insertedId = this.personClass.insertPerson(
          this.personTable[i]["Name"],
          this.personTable[i]["Salary"],
          this.personTable[i]["SocialFactor"],
          this.personTable[i]["IncrementFactor"]
        );
        //update allocation temprory Ids
        for (var j = 0; j < this.allocationTable.length; j++) {
          if (
            this.allocationTable[j]["personId"] == this.personTable[i]["Id"]
          ) {
            this.allocationTable[j]["personId"] = insertedId;
          }
        }
      } else if (
        this.personTable[i]["Flag"] == "U" &&
        this.personTable[i]["Id"].charAt(0) == "T"
      ) {
        this.personClass.insertPerson(
          this.personTable[i]["Name"],
          this.personTable[i]["Salary"],
          this.personTable[i]["SocialFactor"],
          this.personTable[i]["IncrementFactor"]
        ...
        this.personClass.updatePerson(
          this.personTable[i]["Id"],
          this.personTable[i]["Name"],
          this.personTable[i]["Salary"],
          this.personTable[i]["SocialFactor"],
          this.personTable[i]["IncrementFactor"]
    ...
    for (var i = 0; i < this.projectTable.length; i++) {
      if (this.projectTable[i]["Flag"] == "I") {
        var insertedId = this.projectClass.insertProject(
        ...
        for (var j = 0; j < this.allocationTable.length; j++) {
          if (
            this.allocationTable[j]["projectId"] == this.projectTable[i]["Id"]
          ) {
            this.allocationTable[j]["projectId"] = insertedId;
          }
        }
      } else if (
        this.projectTable[i]["Flag"] == "U" &&
        this.projectTable[i]["Id"].charAt(0) == "T"
      ) {
        this.projectClass.insertProject(
          this.projectTable[i]["Name"],
          this.projectTable[i]["EndDate"],
          this.projectTable[i]["Stl"],
          this.projectTable[i]["ExternalSalary"],
        ...
        this.projectClass.updateProject(
          this.projectTable[i]["Id"],
          this.projectTable[i]["Name"],
          this.projectTable[i]["EndDate"],
          this.projectTable[i]["Stl"],
          this.projectTable[i]["ExternalSalary"],
    ...
    for (var i = 0; i < this.allocationTable.length; i++) {
      if (this.allocationTable[i]["Flag"] == "I") {
        this.allocationClass.insertAllocation(
          this.allocationTable[i]["personId"],
          this.allocationTable[i]["projectId"],
          this.allocationTable[i]["Percentage"],
          this.allocationTable[i]["StartDate"],
          this.allocationTable[i]["EndDate"]
        );
      } else if (
        this.allocationTable[i]["Flag"] == "U" &&
        this.allocationTable[i]["Id"].charAt(0) == "T"
      ) {
        this.allocationClass.insertAllocation(
          this.allocationTable[i]["personId"],
          this.allocationTable[i]["projectId"],
        ...
        this.allocationClass.updateAllocation(
          this.allocationTable[i]["Id"],
          this.allocationTable[i]["Percentage"],
          this.allocationTable[i]["StartDate"],
          this.allocationTable[i]["EndDate"]
        );
